...
variable "saml_certificate_end_date" {
  type        = string
  description = "End date for SAML SSO autogenerated certificate (UTC ISO 8601 format, max 3 years from now)"
  validation {
    condition = can(timestamp(var.saml_certificate_end_date))
    error_message = "saml_certificate_end_date must be a valid UTC ISO 8601 timestamp (e.g., 2027-05-08T00:00:00Z)."
  }
}
